...
def _get_log_path():
    """ Get path to logs/run.csv, creating directory if needed """
    log_dir = Path("logs")
...
def _init_csv_if_needed(log_path):
...
def estimate_cost(provider, model, prompt_tokens=None, completion_tokens=None):
    """
...
def log_llm_call(
    ...
    # Estimate cost using actual tokens
